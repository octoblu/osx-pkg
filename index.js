...
function addComponent(inDir, outDir, opts, cb) {
  ...
  function createPayload() {
    ...
    parallel(
      [
        function(cb) {
          pump(pack, zlib.createGzip(), fs.createWriteStream(outDir + "/" + opts.identifier + "/Payload"), cb)
        },
        function(cb) {
  ...
  function createBOMFile() {
    ...
    pump(mkbom(dir, { uid: 0, gid: 80 }), fs.createWriteStream(outDir + "/" + opts.identifier + "/Bom"), cb)
  }
}
